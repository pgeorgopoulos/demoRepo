...
def test_app_file():
    with app.test_client() as c:
        ...
        assert json_data["message"] == "I love to code in " + appenv, 'message output not correct'
        now = time.time()
...
def test_app(ipaddr):
        ...
        json_data = r.json()
        assert json_data["message"] == "Automation for the People", 'message output not correct'
        now = time.time()
        assert json_data["timestamp"] <= now, 'timestamp shows future time'
        assert isinstance(json_data["timestamp"], int), 'timestamp not in correct integer format'
        return 'OK'